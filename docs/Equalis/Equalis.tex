...
\begin{itemize}
  ...
  \item \textbf{Claim: Unified liquidity by token.} For each token, there is a single global pool that aggregates liquidity and fee routing across protocol surfaces.
...
\begin{itemize}
  ...
  \item \textbf{Unified liquidity per token:} enforce one global pool per asset and route fees from multiple protocol surfaces back into the same accounting base.
...
\begin{enumerate}
  \item \textbf{One global pool per token} (unified liquidity and unified fee routing),
...
For each supported token $T$, Equalis maintains a single global pool $\mathcal{P}_T$. All protocol surfaces that use token $T$ reference $\mathcal{P}_T$ as the canonical liquidity source and the canonical destination for fee routing in token $T$.
...
Each token $T$ has exactly one pool $\mathcal{P}_T$. This pool is the shared accounting and liquidity substrate for all actions involving $T$. The pool tracks:
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Unified liquidity:} one pool per token across the entire system.
...
\begin{itemize}
  \item permissionless basket creation (subject to system constraints),
...
Equalis is designed to eliminate a common DeFi tradeoff: increasing product surface area typically fragments liquidity and forces users to choose which venue gets their capital today. Equalis instead enforces \textbf{one global pool per token} and treats internal venues as accounting-level allocations of that pool rather than separate liquidity systems.

\subsection{One Pool per Token (Globally)}

For each token $T$, the protocol maintains a single canonical pool $\mathcal{P}_T$. There are not multiple competing pools for the same token across different protocol products. All modules that use $T$ reference the same pool state and the same fee rails.
...
The unified encumbrance interface allows modules to be composed while maintaining a single global pool per token.
...
Equalis enforces one pool per token. This changes the scaling dynamic:
\begin{itemize}
...
\begin{itemize}
  \item one pool per token prevents liquidity and incentives from fragmenting into silos,
...
\begin{itemize}
  ...
  \item \textbf{Parameters:} deterministic credit bounds (e.g., self-secured LTV $\lambda_T$), fee routing configuration, and module eligibility flags.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Unified liquidity by token:} one global pool per token, with encumbrance-based allocation, prevents liquidity and incentives from fragmenting across products.
...
\begin{description}
  \item[Pool ($\mathcal{P}_T$)] The single global pool for token $T$ that holds unified accounting and fee rails.
